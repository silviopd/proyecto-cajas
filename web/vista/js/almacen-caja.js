...
$("#btnAgregar").click(function(event) {
    ...
    $("#modal_estado").val("H")
    $("#modal_titulo").text("nueva caja");
});

function listar() {

    var ruta = DIRECCION_WS + "caja.almacen.listar.php";
    ...
    $.post(ruta, function() {}).done(function(resultado) {
        ...
        if (datosJSON.estado === 200) {
            ...
            $.each(datosJSON.datos, function(i, item) {
                ...
                html += '<button type="button" class="btn btn-sm btn-icon waves-effect waves-light btn-warning" data-toggle="modal" data-target="#signup-modal" onclick="leerDatos(' + item.id_caja + ')"> <i class="fa fa-wrench"></i> </button>';
                html += '&nbsp;'
...
$("#frmgrabar").submit(function(evento) {
    ...
    if ($("#txttipooperacion").val() == "agregar") {
        ...
        var ruta = DIRECCION_WS + "caja.almacen.agregar.php";
        ...
        var ruta = DIRECCION_WS + "caja.almacen.editar.php";
        var id_caja = $("#modal_id_caja").val()
        var nombre = $("#modal_nombre").val()
        var precio_base = $("#modal_precio_base").val()
        var precio_retraso = $("#modal_precio_retraso").val()
        var estado = $("#modal_estado").val()
        ...
        swal({
            ...
            $.post(ruta, { id_caja: id_caja, nombre: nombre, precio_base: precio_base, precio_retraso: precio_retraso, estado: estado }, function() {}).done(function(resultado) {
                var datosJSON = resultado;
                if (datosJSON.estado === 200) {
                    swal({
                        title: 'EXITO!',
...
function leerDatos(id_caja) {

    var ruta = DIRECCION_WS + "caja.almacen.leerdatos.php";

    $.post(ruta, { id_caja: id_caja }, function() {}).done(function(resultado) {
        var datosJSON = resultado;
        if (datosJSON.estado === 200) {
            $.each(datosJSON.datos, function(i, item) {
                $("#txttipooperacion").val("editar");

                $("#modal_id_caja").val(item.id_caja);
