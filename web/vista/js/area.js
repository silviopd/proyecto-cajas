    ...
    $.post(ruta, function() {}).done(function(resultado) {
        ...
        if (datosJSON.estado === 200) {
            ...
            $.each(datosJSON.datos, function(i, item) {
                ...
                html += '<button type="button" class="btn btn-sm btn-icon waves-effect waves-light btn-warning" data-toggle="modal" data-target="#signup-modal" onclick="leerDatos(' + item.id_area + ')"> <i class="fa fa-wrench"></i> </button>';
                html += '&nbsp;'
...
$("#frmgrabar").submit(function(evento) {
    ...
    if ($("#txttipooperacion").val() == "agregar") {
        ...
        var ruta = DIRECCION_WS + "area.agregar.php";
        var nombre = $("#modal_nombre").val()
        var estado = $("#modal_estado").val()

        swal({
            title: '¿Desea Registrar?',
            text: "se agregará un nuevo area!",
        ...
        var ruta = DIRECCION_WS + "area.editar.php";
        var id_area = $("#modal_id_area").val()
        var nombre = $("#modal_nombre").val()
        var estado = $("#modal_estado").val()

        swal({
            title: '¿Desea Modificar?',
            text: "se modificará el area "+nombre,
            showCancelButton: true,
            confirmButtonClass: 'btn btn-confirm mt-2',
            cancelButtonClass: 'btn btn-cancel ml-2 mt-2',
            confirmButtonText: 'registrar',
            ...
            $.post(ruta, { id_area: id_area, nombre: nombre, estado: estado }, function() {}).done(function(resultado) {
                var datosJSON = resultado;
                if (datosJSON.estado === 200) {
                    swal({
                        title: 'EXITO!',
...
function leerDatos(id_area) {

    var ruta = DIRECCION_WS + "area.leerdatos.php";

    $.post(ruta, { id_area: id_area }, function() {}).done(function(resultado) {
        var datosJSON = resultado;
        if (datosJSON.estado === 200) {
            $.each(datosJSON.datos, function(i, item) {
                $("#txttipooperacion").val("editar");

                $("#modal_id_area").val(item.id_area);
