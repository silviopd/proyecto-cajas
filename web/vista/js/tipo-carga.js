...
$("#btnAgregar").click(function(event) {
    ...
    $("#modal_nombre").val("")
    $("#modal_precio").val("")
    $("#modal_titulo").text("nuevo tipo carga");
});

function listar() {

    var ruta = DIRECCION_WS + "tipo.carga.listar.php";
    ...
    $.post(ruta, function() {}).done(function(resultado) {
        ...
        if (datosJSON.estado === 200) {
            ...
            $.each(datosJSON.datos, function(i, item) {
                ...
                html += '<button type="button" class="btn btn-sm btn-icon waves-effect waves-light btn-warning" data-toggle="modal" data-target="#signup-modal" onclick="leerDatos(' + item.id_tipo_carga + ')"> <i class="fa fa-wrench"></i> </button>';
                html += '&nbsp;'
...
$("#frmgrabar").submit(function(evento) {
    ...
    if ($("#txttipooperacion").val() == "agregar") {
        ...
        var ruta = DIRECCION_WS + "tipo.carga.agregar.php";
        var nombre = $("#modal_nombre").val()
        var precio = $("#modal_precio").val()

        swal({
            title: '¿Desea Registrar?',
            text: "se agregará un nuevo tipo carga!",
        ...
        var ruta = DIRECCION_WS + "tipo.carga.editar.php";
        var id_tipo_carga = $("#modal_id_tipo_carga").val()
        var nombre = $("#modal_nombre").val()
        var precio = $("#modal_precio").val()

        swal({
            title: '¿Desea Modificar?',
            text: "se modificar el tipo carga "+nombre,
            showCancelButton: true,
            confirmButtonClass: 'btn btn-confirm mt-2',
            cancelButtonClass: 'btn btn-cancel ml-2 mt-2',
            confirmButtonText: 'registrar',
            ...
            $.post(ruta, { id_tipo_carga: id_tipo_carga, nombre: nombre, precio: precio }, function() {}).done(function(resultado) {
                var datosJSON = resultado;
                if (datosJSON.estado === 200) {
                    swal({
                        title: 'EXITO!',
...
function leerDatos(id_tipo_carga) {

    var ruta = DIRECCION_WS + "tipo.carga.leerdatos.php";
    ...
    $.post(ruta, { id_tipo_carga: id_tipo_carga }, function() {}).done(function(resultado) {
        ...
        if (datosJSON.estado === 200) {
            $.each(datosJSON.datos, function(i, item) {
                ...
                $("#modal_id_tipo_carga").val(item.id_tipo_carga);
                $("#modal_nombre").val(item.nombre);
                $("#modal_precio").val(item.precio);

                $("#modal_titulo").text("Editar tipo carga");
            });
        } else {
            swal("Mensaje del sistema", resultado, "warning");
        }
